# ---
#   title: "NATCANCER_code_for_figure_5"
...
#data prep
test_2 <- read.csv("../data/oncoprint_pathways_baseline_mat_for_NatCancer.csv")
p_data_ordered <- read.csv("../data/p_data_for_NatCancer.csv") 
...
## Figure 5
#sample split
mat_for_onco_baseline_pathways_ORR_res <- test_2[,colnames(test_2) %in% (subset(p_data_ordered, `CR_PR` == "Yes", select = sample_id_clinsight) %>% pull())]
mat_for_onco_baseline_pathways_ORR_nonres <- test_2[,colnames(test_2) %in% (subset(p_data_ordered, `CR_PR` == "No", select = sample_id_clinsight) %>% pull())]
